...
export default ({ products }: Props) =>
    products.map((p) => (
        <Link href={`/products/${p.id}`} key={p.id} className="flex flex-col gap-2 border-b border-muted pb-1">
            ...
            <div>
                ...
                <div className="flex flex-row justify-between items-center mt-1">
                    ...
                    <p className="w-fit rounded-md bg-muted/65 p-2 font-medium text-xs shadow-sm">
                        {p.category.name}
                    </p>
                </div>
            </div>
